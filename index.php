...
define('SCRIPT_PATH_LOG', 'scripts/git-local-log.sh');
define('SCRIPT_PATH_SHOW', 'scripts/git-local-show.sh');

/* Users */
$Users_file = "scripts/accounts.json";
...
switch ($action) {
    case 'login':
        ...
        if (! isValidUser() && ! empty($_POST)) {
            ...
            $usercheck = 0;
            foreach($Ujson as $accounts){
                foreach($accounts as $user){
                    if(in_array($loginUser, $user)){
                        $usercheck += 1;
                        if($loginPass != $user['password']){
                            setMsg('loginFailed', 'Invalid Password');
                            header('Location: ' . SELF_URL);
                        }else{
                            $_SESSION['Workspace'] = $user['workspace'];
                            $_SESSION['location'] = $user['location'];
                            header('Location: ' . $_SESSION['Workspace']);
                            setValidUser(true);
                        }
                    }
                }
        ...
        if($usercheck == 0){
                setMsg('loginFailed', 'Invalid Username');
            	header('Location: ' . SELF_URL);
        }
        exit;
    }
